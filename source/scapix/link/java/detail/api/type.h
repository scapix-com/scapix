...
struct type
{
	...
	static local_ref<T[]> new_array(jsize len, java::ref<T> init) noexcept { return local_ref<T[]>(env()->NewObjectArray(len, T::class_object().handle(), init.handle())); }
	static local_ref<T> get_array_element(java::ref<T[]> array, jsize index) noexcept { return make_local_ref(env()->GetObjectArrayElement(array.handle(), index)); }
	static void set_array_element(java::ref<T[]> array, jsize index, java::ref<T> value) noexcept { env()->SetObjectArrayElement(array.handle(), index, value.handle()); }

private:

	static local_ref<T> make_local_ref(jobject handle)
	{
		return local_ref<T>(static_cast<typename T::handle_type>(handle));
	}
};
